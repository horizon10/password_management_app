...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    companion object {
        ...
        private const val DATABASE_VERSION = 2 // Versiyonu artırıyoruz
        const val TABLE_NAME = "passwords"
        private const val COLUMN_ID = "id"
        private const val COLUMN_APP_NAME = "app_name"
        const val COLUMN_USERNAME = "username"
        const val COLUMN_PASSWORD = "password"
        private const val COLUMN_APP_TYPE = "app_type" // Yeni sütun
    }
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        try {
            if (oldVersion < 2) {
                // Versiyon güncellemesi sırasında yeni sütunu ekliyoruz
    ...
    fun addPassword(appName: String, username: String, password: String, appType: String): Long {
        ...
        val values = ContentValues().apply {
            ...
            put(COLUMN_APP_TYPE, appType) // Yeni sütunu ekliyoruz
        }
                ...
                do {
                    passwords.add(
                        Password(
                            ...
                            appType = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_APP_TYPE)) // Yeni sütunu ekliyoruz
                        )
                    )
